...
const TOKEN_NAME: string = "Fame Global";
const TOKEN_SYMBOL: string = "FAMEG";
const TOTAL_SUPPLY: string = "1000000000000000000000000000";

describe(TOKEN_SYMBOL + " Token Test", function () {
    let contract: FAMEG;
    let supervisorContract: FAMEG;
    let ordinaryContract: FAMEG;
    let freezedContract: FAMEG;
    let burnerContract: FAMEG;
    let lockerContract: FAMEG;
    let timeLocked1Contract: FAMEG;
    let timeLocked2Contract: FAMEG;
    let timeLocked3Contract: FAMEG;
    let vestingLocked1Contract: FAMEG;
    let vestingLocked2Contract: FAMEG;
    let owner: string;
    ...
    beforeEach(async function () {
        const [
            ...
            timeLocked2Signer,
            timeLocked3Signer,
            vestingLocked1Signer,
            vestingLocked2Signer,
        ] = await ethers.getSigners();

        owner = ownerSigner.address;
        supervisor = supervisorSigner.address;
        ordinary = ordinarySigner.address;
        freezed = freezedSigner.address;
        burner = burnerSigner.address;
        locker = lockerSigner.address;
        timeLocked1 = timeLocked1Signer.address;
        timeLocked2 = timeLocked2Signer.address;
        timeLocked3 = timeLocked3Signer.address;
        vestingLocked1 = vestingLocked1Signer.address;
        vestingLocked2 = vestingLocked2Signer.address;

        const Contract = await ethers.getContractFactory(TOKEN_SYMBOL);
        contract = (await Contract.connect(ownerSigner).deploy()) as FAMEG;
        supervisorContract = contract.connect(supervisorSigner);
        ordinaryContract = contract.connect(ordinarySigner);
        freezedContract = contract.connect(freezedSigner);
        burnerContract = contract.connect(burnerSigner);
        lockerContract = contract.connect(lockerSigner);
        timeLocked1Contract = contract.connect(timeLocked1Signer);
        timeLocked2Contract = contract.connect(timeLocked2Signer);
        timeLocked3Contract = contract.connect(timeLocked3Signer);
        vestingLocked1Contract = contract.connect(vestingLocked1Signer);
        vestingLocked2Contract = contract.connect(vestingLocked2Signer);

        await contract.transferSupervisorOwnership(supervisor);
    });

    describe("1. basic info test", async () => {
        it("1-1 should basic info right", async () => {
    ...
    describe("2. transfer test", async () => {
